...
"use strict";class DSHandler{constructor(r){this.browser=r,this.util=r.util}getDSNodeForPath(){var r=this.browser.ds["@graph"][0],t={type:"",node:{}};if(this.browser.ds)if(this.browser.path){for(var e=this.browser.path.split("-"),s=0;s<e.length;s++)""!==e[s]&&(e[s].charAt(0).toUpperCase()===e[s].charAt(0)?null!==r&&(r=this.getClass(r["sh:or"],e[s])):null!==r&&s!==e.length-1&&(r=void 0!==r["sh:targetClass"]?this.getProperty(r["sh:property"],e[s]):this.getProperty(r["sh:node"]["sh:property"],e[s])));if(r&&r["sh:class"]&&!Array.isArray(r["sh:class"]))try{this.browser.sdoAdapter.getEnumeration(r["sh:class"]),t.type="Enumeration"}catch(r){t.type="Class"}else t.type="Class"}else t.type="Class";else t.type="error";return t.node=r,t}getClass(r,t){for(var e=0;e<r.length;e++)if(void 0!==r[e]["sh:class"]&&this.rangesToString(r[e]["sh:class"])===t)return r[e];return null}getProperty(r,t){for(var e=0;e<r.length;e++)if(this.rangesToString(r[e]["sh:path"])===t)return r[e];return null}dataTypeMapperFromSHACL(r){switch(r){case"xsd:string":return"http://schema.org/Text";case"xsd:boolean":return"http://schema.org/Boolean";case"xsd:date":return"http://schema.org/Date";case"xsd:dateTime":return"http://schema.org/DateTime";case"xsd:time":return"http://schema.org/Time";case"xsd:double":return"http://schema.org/Number";case"xsd:float":return"http://schema.org/Float";case"xsd:integer":return"http://schema.org/Integer";case"xsd:anyURI":return"http://schema.org/URL"}return null}rangesToString(r){return Array.isArray(r)?r.map(r=>this.util.prettyPrintIri(r)).join(" + "):this.util.prettyPrintIri(r)}}module.exports=DSHandler;

},{}],86:[function(require,module,exports){
"use strict";class DSRenderer{constructor(e){this.browser=e,this.util=e.util,this.dsHandler=e.dsHandler}renderShacl(){this.browser.elem.innerHTML='<pre style="font-size: medium; background: none; text-align: left; width: auto; padding: 0; overflow: visible; color: rgb(0, 0, 0); line-height: normal; display: block; font-family: monospace; margin: 1em 0; word-wrap: break-word; white-space: pre-wrap;">'+JSON.stringify(this.browser.ds,null,2)+"</pre>"}render(){this.dsNode=this.browser.dsNode,this.node=this.dsNode.node;var e=this.createHeader()+("Class"===this.dsNode.type?this.createClassPropertyTable():this.createEnumerationMembers());this.browser.elem.innerHTML=this.util.createMainContent("rdfs:Class",e)}createHeader(){var e,r,t="";if(this.browser.path){var s=this.node["sh:class"];e=this.util.prettyPrintIri(s),r=this.browser.sdoAdapter.getTerm(s).getDescription(),t=this.createBreadcrumbs()}else{var i=this.browser.ds["@graph"][0];e=i["schema:name"]||"Domain Specification",r=i["schema:description"]||""}return r=this.util.repairLinksInHTMLCode(r),this.createNavigation()+'<h1 property="schema:name">'+e+"</h1>"+this.util.createExternalLinkLegend()+t+'<div property="schema:description">'+r+"<br><br></div>"}createBreadcrumbs(){return'<h4><span class="breadcrumbs">'+this.util.createJSLink("path",null,this.browser.ds["@graph"][0]["schema:name"]||"Domain Specification")+" > "+this.browser.path.split("-").map((e,r,t)=>{if(r%2==0)return e;var s=t.slice(0,r+1).join("-");return this.util.createJSLink("path",s,e)}).join(" > ")+"</span></h4>"}createNavigation(){return'<span style="float: right;">('+this.util.createJSLink("format","shacl","SHACL serialization")+(this.browser.list?" | from List: "+this.util.createJSLink("ds",null,this.browser.list["schema:name"]):"")+")</span>"}createClassPropertyTable(){var e=(this.browser.path?this.node["sh:node"]["sh:property"].slice(0):this.node["sh:property"].slice(0)).map(e=>this.createClassProperty(e)).join("");return this.util.createDefinitionTable(["Property","Expected Type","Description","Cardinality"],e)}createClassProperty(e){var r=e["sh:path"],t=this.browser.sdoAdapter.getProperty(r);return this.util.createTableRow("rdf:Property",t.getIRI(),"rdfs:label",this.util.createTermLink(r),this.createClassPropertySideCols(e),"prop-name")}createClassPropertySideCols(e){return'<td class="prop-ect">'+this.createExpectedTypes(e)+'</td><td class="prop-desc">'+this.createClassPropertyDescText(e)+'</td><td class="prop-ect">'+this.createCardinality(e)+"</td>"}createClassPropertyDescText(e){var r=this.util.prettyPrintIri(e["sh:path"]),t="";try{t=this.browser.sdoAdapter.getProperty(r).getDescription()}catch(e){}var s=e["rdfs:comment"]?e["rdfs:comment"]:"",i="";return""!==t&&(""!==s&&(i+="<b>From Vocabulary:</b> "),i+=t),""!==s&&(""!==t&&(i+="<br><b>From Domain Specification:</b> "),i+=s),this.util.repairLinksInHTMLCode(i)}createExpectedTypes(e){var r=this.browser.sdoAdapter.getProperty(e["sh:path"]),t=this.util.prettyPrintIri(r.getIRI(!0)),s=e["sh:or"],i="";return s.forEach(e=>{var r;e["sh:datatype"]?r=e["sh:datatype"]:e["sh:class"]&&(r=e["sh:class"]);var s=this.dsHandler.dataTypeMapperFromSHACL(r);if(null!==s)i+=this.util.createLink(s);else{r=this.dsHandler.rangesToString(r);var a=t+"-"+r;i+=this.util.createJSLink("path",a,r,null,"-")}i+="<br>"}),i}createCardinality(e){var r,t=e["sh:minCount"],s=e["sh:maxCount"],i="";return t&&0!==t?s&&0!==s?t!==s?(r="This property is required. It must have between "+t+" and "+s+" value(s).",i=t+".."+s):(r="This property is required. It must have "+t+" value(s).",i=t):(r="This property is required. It must have at least "+t+" value(s).",i=t+"..N"):s&&0!==s?(r="This property is optional. It must have at most "+s+" value(s).",i="0.."+s):(r="This property is optional. It may have any amount of values.",i="0..N"),'<span title="'+r+'">'+i+"</span>"}createEnumerationMembers(){var e=this.browser.sdoAdapter.getTerm(this.node["sh:class"]).getEnumerationMembers();return 0!==e.length?'An Enumeration with:<br><b><a id="enumbers" title="Link: #enumbers" href="#enumbers" class="clickableAnchor">Enumeration members</a></b><ul>'+e.map(e=>{var r=this.browser.sdoAdapter.getEnumerationMember(e);return"<li>"+this.util.createLink(r.getIRI(),e)+"</li>"}).join("")+"</ul><br>":""}}module.exports=DSRenderer;
