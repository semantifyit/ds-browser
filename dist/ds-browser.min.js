...
const SDOAdapter=require("schema-org-adapter"),DsUtilities=require("ds-utilities"),DSBrowserIFrameContent=require("./templates/DSBrowserIFrameContent.js"),Util=require("./Util"),DSHandler=require("./DSHandler"),SharedFunctions=require("./SharedFunctions.js"),ListView=require("./views/ListView.js"),SHACLView=require("./views/SHACLView.js"),NativeView=require("./views/NativeView.js"),TreeView=require("./views/TreeView.js");class DSBrowser{constructor(t){this.dsCache={},this.sdoCache=[],this.util=new Util(this),this.dsHandler=new DSHandler(this),this.sharedFunctions=new SharedFunctions(this),this.listView=new ListView(this),this.shaclView=new SHACLView(this),this.nativeView=new NativeView(this),this.treeView=new TreeView(this),this.editFunction=t.editFunction,this.targetElement=t.targetElement,this.locationControl=!1!==t.locationControl,this.selfFileHost=!0===t.selfFileHost,this.locationControl?this.readStateFromUrl():(this.listId=t.listId||null,this.dsId=t.dsId||null,this.path=t.path||null,this.viewMode=t.viewMode||null,this.format=t.format||null),this.frameSize=this.targetElement.style.height?this.targetElement.style.height:"100vh",this.locationControl&&window.addEventListener("popstate",async()=>{this.readStateFromUrl(),await this.render()}),this.initIFrame(),console.log(this)}async initIFrame(){this.targetElement.innerHTML=this.frame(),this.dsbFrame=document.getElementById("ds-browser-iframe"),this.dsbFrame.style.height=this.frameSize,this.dsbContext=this.dsbFrame.contentWindow.document,this.dsbContext.open(),this.dsbContext.write(DSBrowserIFrameContent),this.dsbContext.close(),setTimeout(function(){this.dsbContainer=this.dsbContext.getElementById("ds-browser-main-container"),this.render()}.bind(this),250)}frame(){return'<div><iframe id="ds-browser-iframe" width="100%" frameborder="0" scrolling="no"></iframe></div>'}async render(){if(this.dsbContainer.innerHTML=this.util.createHtmlLoading(),await this.renderInit(),"shacl"===this.format)this.shaclView.render();else if(this.dsId&&"tree"!==this.viewMode&&"table"!==this.viewMode)this.nativeView.render();else if(this.dsId&&"tree"===this.viewMode)this.treeView.render();else{if(!this.listId)throw new Error("Input parameters invalid.");this.listView.render()}this.addJSLinkEventListener(),document.body.scrollTop=document.documentElement.scrollTop=0}async renderInit(){!this.listId||this.list&&this.list["@id"].endsWith(this.listId)||await this.initList(),!this.dsId||this.ds&&this.util.getDSRootNode(this.ds)["@id"].endsWith(this.dsId)||await this.initDS(),this.ds&&(this.dsRootNode=this.util.getDSRootNode(this.ds),this.dsNode=this.dsHandler.getDSNodeForPath())}async initList(){this.list=await this.util.parseToObject(this.util.getFileHost()+"/list/"+this.listId+"?representation=lean")}async initDS(){if(this.dsCache[this.dsId])this.ds=this.dsCache[this.dsId].populated,this.dsUnpopulated=this.dsCache[this.dsId].unpopulated;else{let t,s=await this.util.parseToObject(this.util.getFileHost()+"/ds/"+this.dsId+"?populate=true");"5.0"===DsUtilities.getDsUtilitiesForDs(s).getDsSpecificationVersion(s)?(s=await this.util.parseToObject(this.util.getFileHost()+"/api/v2/domainspecifications/dsv7/"+this.dsId+"?populate=true"),this.dsUnpopulated=null):t=await this.util.parseToObject(this.util.getFileHost()+"/ds/"+this.dsId+"?populate=false"),this.dsCache[this.dsId]={},this.dsCache[this.dsId].populated=s,this.dsCache[this.dsId].unpopulated=t,this.ds=s,this.dsUnpopulated=t}this.sdoAdapter||(this.sdoAdapter=new SDOAdapter({schemaHttps:!0}));const t=await this.getVocabUrlsForDS();let s=this.util.getSdoAdapterFromCache(t);s||(s=new SDOAdapter({schemaHttps:!0,equateVocabularyProtocols:!0}),await s.addVocabularies(t),this.sdoCache.push({vocabUrls:t,sdoAdapter:s})),this.dsUtil=DsUtilities.getDsUtilitiesForDs(this.ds),this.sdoAdapter=s}async getVocabUrlsForDS(){if(!this.ds)return[];let t=[];const s=this.util.getDSRootNode(this.ds);if(s&&Array.isArray(s["ds:usedVocabulary"])&&(t=this.util.cloneJson(s["ds:usedVocabulary"])),s&&s["schema:schemaVersion"]){const i=this.getSDOVersion(s["schema:schemaVersion"]);t.push(await this.sdoAdapter.constructSDOVocabularyURL(i))}return t}getSDOVersion(t){if(t.startsWith("http")){let s=/.*schema\.org\/version\/([0-9.]+)\//g.exec(t);if(s&&s[1])return s[1]}return t}addJSLinkEventListener(){const t=this.dsbContext.getElementsByClassName("a-js-link");for(const s of t)s.addEventListener("click",async t=>{this.locationControl?t.ctrlKey?window.open(s.href):(history.pushState(null,null,s.href),this.navigate(JSON.parse(decodeURIComponent(s.getAttribute("data-state-changes"))))):this.navigate(JSON.parse(decodeURIComponent(s.getAttribute("data-state-changes"))))})}readStateFromUrl(){const t=new URLSearchParams(window.location.search);if(this.path=t.get("path")||null,this.path&&this.path.includes("   ")&&(this.path=this.path.replace(new RegExp(" {3}","g")," + ")),this.format=t.get("format")||null,this.viewMode=t.get("mode")||null,window.location.pathname.includes("/ds/")){this.listId=null;let t=window.location.pathname.substring("/ds/".length);this.dsId!==t&&(this.dsId=t,this.ds=null,this.dsUnpopulated=null)}else if(window.location.pathname.includes("/list/")){let s=window.location.pathname.substring("/list/".length);this.listId!==s&&(this.listId=s,this.list=null);let i=t.get("ds")||null;this.dsId!==i&&(this.dsId=i,this.ds=null,this.dsUnpopulated=null)}else this.listId=null,this.list=null,this.dsId=null,this.ds=null,this.dsUnpopulated=null}navigate(t){void 0!==t.listId&&(this.listId=t.listId),void 0!==t.dsId&&(this.dsId=t.dsId),void 0!==t.path&&(this.path=t.path),void 0!==t.viewMode&&(this.viewMode=t.viewMode),void 0!==t.format&&(this.format=t.format),null===this.listId&&(this.list=void 0),null===this.dsId&&(this.ds=void 0,this.dsUnpopulated=void 0),this.ds||(this.path=null),this.render()}}module.exports=DSBrowser;
...
const ListTableDsRow=require("../templates/ListTableDsRow.js");class ListView{constructor(t){this.b=t}render(){let t=this.getFrame();t=(t=(t=t.replace(/{{navigationBar}}/g,this.b.sharedFunctions.getNavigationBarHTML())).replace(/{{listTitleName}}/g,this.b.list["schema:name"]||"List")).replace(/{{listDescription}}/g,this.b.list["schema:description"]||"No List description available.");const e="<tbody>"+this.b.list["schema:hasPart"].map(t=>this.getDsRowHTML(t)).join("")+"</tbody>";t=t.replace(/{{tableBody}}/g,e),this.b.dsbContainer.innerHTML=t}getFrame(){return'<div id="list-view" class="noNav">\n      \x3c!--Page content--\x3e\n      <div class="container-xl">\n        \x3c!--Title--\x3e\n        <div class="row">\n          <div class="col-12 mt-4 text-dark">\n            <h2 class="mb-0">\n              <span>{{listTitleName}}</span>\n            </h2>\n          </div>\n          <div class="col-12 mb-3 mt-2">\n             {{listDescription}}\n          </div>\n        </div>\n        \x3c!--Table for DS node Content  --\x3e\n        <table class="table bg-light rounded shadow-2">\n           <thead>\n              <tr class="bg-primary text-white align-middle">\n                <th scope="col">Domain Specification</th>\n                <th scope="col">Description</th>\n              </tr>\n           </thead>    \n           {{tableBody}}\n        </table>\n      </div>\n    </div>\n'}getDsRowHTML(t){const e=t["schema:name"]||"DS with no name",i=`<a class="a-js-link" ${this.b.util.createInternalLinkAttributes({dsId:t["@id"].substring(t["@id"].lastIndexOf("/")+1)})} >${e}</a>`,s=this.b.util.repairLinksInHTMLCode(t["schema:description"]);return ListTableDsRow.replace(/{{ds}}/g,i).replace(/{{details}}/g,s)}}module.exports=ListView;

},{"../templates/ListTableDsRow.js":100}],107:[function(require,module,exports){
const DSTitleFrame=require("../templates/DsTitle.js"),NativeTableClassHeader=require("../templates/NativeTableClassHeader.js"),NativeTableClassRow=require("../templates/NativeTableClassRow.js"),NativeTableEnumerationHeader=require("../templates/NativeTableEnumerationHeader.js"),NativeTableEnumerationRow=require("../templates/NativeTableEnumerationRow.js");class NativeView{constructor(e){this.b=e}render(){let e=this.getFrame();if(e=e.replace(/{{navigationBar}}/g,this.b.sharedFunctions.getNavigationBarHTML()),e=(e=this.b.sharedFunctions.setDsTitleHtml(e)).replace(/{{dsPathBreadcrumbs}}/g,this.getDsPathBreadcrumbsHTML()),"Class"===this.b.dsNode.type){e=e.replace(/{{tableHead}}/g,NativeTableClassHeader);const t="<tbody>"+this.b.dsNode.node["sh:property"].map(e=>this.getPropertyRowHTML(e)).join("")+"</tbody>";e=e.replace(/{{tableBody}}/g,t)}else{e=e.replace(/{{tableHead}}/g,NativeTableEnumerationHeader);const t="<tbody>"+this.b.dsNode.node["sh:in"].map(e=>this.getEnumerationMemberRowHTML(e)).join("")+"</tbody>";e=e.replace(/{{tableBody}}/g,t)}this.b.dsbContainer.innerHTML=e}getFrame(){return`<div id="native-view" class="withNav">\n      \x3c!--Navigation Bar--\x3e\n      {{navigationBar}}\n      \x3c!--Page content--\x3e\n      <div class="container-xl">\n        \x3c!--Title--\x3e\n        ${DSTitleFrame}\n        \x3c!--Breadcrumbs for DS Path--\x3e\n        <div class="card my-3 shadow-2">\n          <div class="card-body d-flex p-2 align-items-center">\n            <div title="Current path within the Domain Specification" class="align-items-center px-2">\n              <span><i class="fas fa-code-branch"></i></span>\n            </div>\n            <div id="native-path" class="align-items-center">{{dsPathBreadcrumbs}}</div>\n          </div>\n        </div>\n        \x3c!--Table for DS node Content  --\x3e\n        <table id="native-table" class="table bg-light rounded shadow-2">\n          {{tableHead}}\n          {{tableBody}}\n        </table>\n      </div>\n    </div>`}getDsPathBreadcrumbsHTML(){let e=this.b.util.createInternalLink({path:null},this.b.dsHandler.rangesToString(this.b.dsRootNode["sh:targetClass"]));return this.b.path&&(e=e+" > "+this.b.path.split("-").map((e,t,a)=>{if(t%2==0)return e;{const r=a.slice(0,t+1).join("-");return this.b.util.createInternalLink({path:r},e)}}).join(" > ")),` <span class="breadcrumbs">${e}</span>`}getPropertyRowHTML(e){let t="";this.isPropertyFromSuperDs(e)&&(t=' <i class="fas fa-angle-double-down" title="This property originates from the Super-DS"></i>');const a=this.b.util.createTermLink(e["sh:path"])+t,r=this.createHtmlExpectedTypes(e),i=this.createHtmlPropertyDescription(e),s=this.b.dsHandler.createHtmlCardinality(e["sh:minCount"],e["sh:maxCount"]);return NativeTableClassRow.replace(/{{property}}/g,a).replace(/{{cardinality}}/g,s).replace(/{{expectedType}}/g,r).replace(/{{details}}/g,i)}isPropertyFromSuperDs(e){if(null!==this.b.path||!this.b.dsUnpopulated)return!1;return void 0!==this.b.dsUtil.getDsRootNode(this.b.dsUnpopulated)["sh:property"].find(t=>t["sh:path"]===e["sh:path"])}getEnumerationMemberRowHTML(e){const t=this.b.sdoAdapter.getEnumerationMember(e["@id"]),a=this.b.util.createTermLink(e["@id"]),r=this.b.util.repairLinksInHTMLCode(t.getDescription());return NativeTableEnumerationRow.replace(/{{enumerationMember}}/g,a).replace(/{{details}}/g,r)}createHtmlExpectedTypes(e){const t=this.b.sdoAdapter.getProperty(e["sh:path"]),a=this.b.util.prettyPrintIri(t.getIRI(!0));return e["sh:or"].map(e=>{let t,r=this.b.util.getClassNodeIfExists(e);e["sh:datatype"]?t=e["sh:datatype"]:r&&r["sh:class"]&&(t=r["sh:class"]);const i=this.b.dsHandler.dataTypeMapperFromSHACL(t);if(null!==i){let e=null;return"rdf:langString"===t?e="Localized Text":"rdf:HTML"===t&&(e="HTML Text"),this.b.util.createLink(i,e)}if(t=this.b.dsHandler.rangesToString(t),r&&Array.isArray(r["sh:property"])&&0!==r["sh:property"].length){const e=this.b.path?this.b.path+"-"+a+"-"+t:a+"-"+t;return this.b.util.createInternalLink({path:e},t)}if(r&&r["sh:class"]&&Array.isArray(r["sh:in"])){const e=this.b.path?this.b.path+"-"+a+"-"+t:a+"-"+t;return this.b.util.createInternalLink({path:e},t)}return this.b.util.createTermLink(t)}).join("<br>")}createHtmlPropertyDescription(e){const t=this.b.util.prettyPrintIri(e["sh:path"]);let a;try{a=this.b.sdoAdapter.getProperty(t).getDescription()}catch(e){a=""}const r=e["rdfs:comment"]?this.b.util.getLanguageString(e["rdfs:comment"]):"";let i="";return""!==a&&(""!==r&&(i+="<b>From Vocabulary:</b> "),i+=a),""!==r&&(""!==a&&(i+="<br><b>From Domain Specification:</b> "),i+=r),this.b.util.repairLinksInHTMLCode(i)}}module.exports=NativeView;
...
const formatHighlight=require("json-format-highlight");class SHACLView{constructor(e){this.b=e}render(){const e=formatHighlight(this.b.ds,{keyColor:"black",numberColor:"#3A01DC",stringColor:"#DF0002",trueColor:"#C801A4",falseColor:"#C801A4",nullColor:"cornflowerblue"});let r=this.getFrame();r=r.replace(/{{preContent}}/g,e),this.b.dsbContainer.innerHTML=r}getFrame(){return'<div id="shacl-view" class="noNav" >\n      <pre style="\n        font-size: large;\n        text-align: left;\n        width: auto;\n        background-color: whitesmoke;\n        padding: 15px 20px;\n        margin: 0;\n        overflow: visible;\n        line-height: normal;\n        display: block;\n        font-family: monospace;\n        word-wrap: break-word;\n        white-space: pre-wrap;\n        ">{{preContent}}</pre>\n    </div>'}}module.exports=SHACLView;

},{"json-format-highlight":42}],109:[function(require,module,exports){
const DSTitleFrame=require("../templates/DsTitle.js");class TreeView{constructor(e){this.b=e,this.dsHandler=this.b.dsHandler}render(){let e=this.getFrame();e=e.replace(/{{navigationBar}}/g,this.b.sharedFunctions.getNavigationBarHTML()),e=this.b.sharedFunctions.setDsTitleHtml(e),this.b.dsbContainer.innerHTML=e,this.initIFrameForJSTree()}getFrame(){return`<div id="tree-view" class="withNav">\n      \x3c!--Navigation Bar--\x3e\n      {{navigationBar}}\n      \x3c!--Page content--\x3e\n      <div class="container-xl">\n        \x3c!--Title--\x3e\n        ${DSTitleFrame}\n        \x3c!--tree iframe--\x3e\n        <div id="div-iframe">\n          <iframe id="iframe-jsTree" frameborder="0" width="100%" scrolling="no"></iframe>\n        </div>\n      </div>\n    </div>`}initIFrameForJSTree(){this.iFrame=this.b.dsbContext.getElementById("iframe-jsTree"),this.iFrameCW=this.iFrame.contentWindow;const e=this.iFrameCW.document,t=this.createJSTreeHTML();e.open(),e.write(t),e.close();const r=this.dsHandler.generateDsClass(this.b.dsRootNode,!1,!1);this.mapNodeForJSTree([r])}createJSTreeHTML(){return`<head>\n            <script src="https://code.jquery.com/jquery-3.6.0.min.js" crossorigin="anonymous"><\/script>\n            <script src="https://cdnjs.cloudflare.com/ajax/libs/jstree/3.3.10/jstree.min.js"><\/script>\n            <script src="https://cdnjs.cloudflare.com/ajax/libs/jstreegrid/3.10.2/jstreegrid.min.js"><\/script>\n            <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css" />\n            <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jstree/3.3.10/themes/default/style.min.css" />\n            ${this.createTreeStyle()}\n            </head>\n            <body><div id="jsTree"></div></body>`}createTreeStyle(){return"<style>\n            .optional-property { color: #ffa517; }\n            .mandatory-property { color: #00ce0c; }\n            </style>"}mapNodeForJSTree(e){const t=this;this.iFrame.addEventListener("load",function(){t.iFrameCW.$("#jsTree").jstree({plugins:["grid"],core:{themes:{icons:!0,dots:!0,responsive:!0,stripes:!0,rootVisible:!1},data:e},grid:{columns:[{width:"20%",header:"Class / Property"},{header:"Range / Type",width:"20%",value:function(e){return e.data.dsRange}},{width:"17%",header:"Cardinality",value:function(e){if(e.data.dsRange)return'<p style="width: 100%; margin: 0; text-align: center; padding-right: 7px;">'+t.dsHandler.createHtmlCardinality(e.data.minCount,e.data.maxCount)+"</p>"}},{width:"50%",header:"Description",value:function(e){return'<p style="width: 100%; overflow: hidden; margin: 0; text-overflow: ellipsis;">'+e.data.dsDescription.replaceAll("</br>"," ")+"</p>"}}]}}).bind("ready.jstree after_open.jstree after_close.jstree refresh.jstree",t.adaptIframe.bind(t)),t.addIframeClickEvent()})}adaptIframe(){const e=window.scrollY,t=window.scrollX;this.iFrame.height="0px",this.iFrame.height=this.iFrameCW.document.body.scrollHeight,window.scrollTo(t,e)}addIframeClickEvent(){this.iFrameCW.$(".btn-vis-shadow").click(e=>{const t=this.iFrameCW.$(e.currentTarget);let r,i;"btn-opt"===t.attr("id")?(r=this.iFrameCW.$("#btn-man"),i=!0):(r=this.iFrameCW.$("#btn-opt"),i=!1),t.off("click"),this.addIframeClickEvent();const s=this.dsHandler.generateDsClass(this.b.dsRootNode,!1,i),a=this.iFrameCW.$("#jsTree").jstree(!0);a.settings.core.data=s,a.refresh()})}}module.exports=TreeView;
